    ...
    while (1)
    {
        ...
        else
        {
                                          ...
                                          BUFFER_SIZE - total_received - 1, 0)) > 0)
            {
                ...
                // 헤더가 아직 완성되지 않았다면 헤더 끝 찾기
                if (!header_complete && (header_end = strstr(buffer, "\r\n\r\n")) != NULL)
                {
                ...
                {
                    if (send(client_socket, buffer, total_received, 0) < 0)
                    {
            ...
            // 남은 데이터 전송
            if (total_received > 0 && request_success)
            {
                if (send(client_socket, buffer, total_received, 0) < 0)
                {
                    log_message(LOG_ERROR, "Failed to send final response to client");
                    request_success = false;
                }
            }
        }
