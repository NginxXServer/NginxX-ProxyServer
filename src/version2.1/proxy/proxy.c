...
/**
 * 백엔드 서버 선택 함수 (라운드 로빈 방식) -> 이 부분 주석 형태는 유지하면서 로직이랑 다 바꿔주세요
...
int select_server() {
    ...
    // current_server 변수 업데이트를 mutex로 보호
    ...
    // 사용 가능한 서버 찾기 (health check 포함)
    int check_idx = start_idx;
    do {
        // is_server_available 내부에서 server_mutex를 사용하여 
        // health check 정보 접근을 보호
        if (is_server_available(&pool, check_idx)) {
            selected = check_idx;
        ...
        check_idx = (check_idx + 1) % MAX_BACKENDS;
